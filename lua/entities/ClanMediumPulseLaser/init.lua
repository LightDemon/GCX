...
function ENT:Initialize()  
	...
	util.PrecacheSound( "Lasers/MPulse/PulseLaser.wav" ) 
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "ClanMediumPulseLaser" )
		ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	if (self.ammos > 0) then
		...
		if (tr.Hit && tr.Entity:IsValid()) then
			if ( !tr.Entity:IsWorld() || !tr.Entity:IsPlayer() || !tr.Entity:IsNPC() || !tr.Entity:IsVehicle()) then
					gcombat_nrghit( tr.Entity, 45, 7, tr.HitPos, tr.HitPos )
		...
		util.Effect( "ClanMediumPulseBeam", effectdata )
		self.Entity:EmitSound( "Lasers/MPulse/PulseLaser.wav", 500, 100 )
