...
self.ticking = true
self.smoking = false
self.flightvector = self.Entity:GetUp() * 120
...
function ENT:Think()
	...
	if (self.smoking == false) then
		...
		FireTrail:SetKeyValue("startwidth","2")
		FireTrail:SetKeyValue("endwidth","2")
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			...
			if (tr.Entity:IsValid()) then
					...
					local attack = cbt_dealhcghit( tr.Entity, 100, 8, tr.HitPos , tr.HitPos)
						if (attack == 0) then
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-0.04,0.04), math.Rand(-0.04,0.04),math.Rand(-0.04,0.04)) + Vector(0,0,-0.05)
