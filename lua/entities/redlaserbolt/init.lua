...
self.Entity:SetModel( "models/peniscolada/redlaser.mdl" ) 	
...
function ENT:Think()
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			...
			if (tr.Entity:IsValid()) then
					...
					local attack = cbt_dealhcghit( tr.Entity, 300, 6, tr.HitPos , tr.HitPos)
						if (attack == 0) then
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-0.05,0.05), math.Rand(-0.05,0.05),math.Rand(-0.05,0.05)) + Vector(0,0,-0)
