...
self.Entity:SetSolid( SOLID_VPHYSICS )        -- CHEESECAKE!    >:3            
 ...
 if (self.smoking == false) then
		...
		FireTrail:SetKeyValue("startwidth","30")
		FireTrail:SetKeyValue("endwidth","30")
		FireTrail:SetKeyValue("spritename","trails/tube.vmt")
	...
	local trace = {}
		...
		trace.endpos = self.Entity:GetPos() + (self.flightvector * 1)
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			...
			local attack = cbt_dealhcghit( tr.Entity, 700, 7, tr.HitPos, tr.HitPos)
				if (attack == 0) then
					local brokedshell = ents.Create("prop_physics")
