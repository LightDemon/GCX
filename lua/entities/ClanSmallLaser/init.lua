...
function ENT:Initialize()   
	...
	util.PrecacheSound( "Lasers/Small/Laser.wav" ) 
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "ClanSmallLaser" )
		ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	if (self.ammos > 0) then
		...
		if (tr.Hit && tr.Entity:IsValid()) then
			if ( !tr.Entity:IsWorld() || !tr.Entity:IsPlayer() || !tr.Entity:IsNPC() || !tr.Entity:IsVehicle()) then
				gcombat.nrghit( tr.Entity, 60, 7, tr.HitPos, tr.HitPos)
		...
		self.reloadtime = CurTime() + 1.75
		
		local effectdata = EffectData()
		effectdata:SetOrigin(tr.HitPos)
		effectdata:SetStart(self.Entity:GetPos() +  self.Entity:GetUp() * 50)
		util.Effect( "ClanSmallLaserBeam", effectdata )
		self.Entity:EmitSound( "Lasers/Small/Laser.wav", 500, 100 )
