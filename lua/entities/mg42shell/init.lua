...
function ENT:Think()
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			...
			if (tr.Entity:IsValid()) then
					...
					local attack = cbt_dealhcghit( tr.Entity, 40, 100, 7, tr.HitPos , tr.HitPos)
						if (attack == 0) then
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-0.15,0.15), math.Rand(-0.15,0.15),math.Rand(-0.15,0.15)) + Vector(0,0,-0.05)
