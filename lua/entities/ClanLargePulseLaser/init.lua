...
function ENT:Initialize()  
	...
	util.PrecacheSound( "Lasers/LPulse/PulseLaser.wav" ) 
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "ClanLargePulseLaser" )
		ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	if (self.ammos > 0) then
		...
		if (tr.Hit && tr.Entity:IsValid()) then
			if ( !tr.Entity:IsWorld() || !tr.Entity:IsPlayer() || !tr.Entity:IsNPC() || !tr.Entity:IsVehicle()) then
				gcombat.nrghit( tr.Entity, 75, 7, tr.HitPos, tr.HitPos)
		...
		util.Effect( "ClanLargePulseBeam", effectdata )
