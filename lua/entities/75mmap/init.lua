	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			if ( self.exploded == false && self.ticking == true ) then
				...
				local attack = gcombat.hcghit( tr.Entity, 3000, 9, tr.HitPos, (self.flightvector * 2))
				if (attack == 0) then
...
self.flightvector = self.flightvector + Vector(math.Rand(-0.10,0.10), math.Rand(-0.1,0.1),math.Rand(-0.1,0.1)) + Vector(0,0,-0.1)
