...
function ENT:Think()
	...
	if (self.smoking == false) then
		...
		FireTrail:SetKeyValue("startwidth","4")
		FireTrail:SetKeyValue("endwidth","4")
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			...
			if (tr.Entity:IsValid()) then
					...
					local attack = cbt_dealhcghit( tr.Entity, 700, 8, tr.HitPos , tr.HitPos)
						if (attack == 0) then
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-0.15,0.15), math.Rand(-0.15,0.15),math.Rand(-0.15,0.15)) + Vector(0,0,-0.1)
