...
self.Entity:SetSolid( SOLID_VPHYSICS )        -- CHEESECAKE!    >:3            
 ...
 function ENT:Think()
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			if ( self.exploded == false && self.ticking == true ) then
				...
				local attack = gcombat.hcghit( tr.Entity, 1100, 9, tr.HitPos, (self.flightvector * 2))
				if (attack == 0) then
	...
	self.flightvector = self.flightvector + Vector(math.Rand(-0.12,0.12), math.Rand(-0.12,0.12),math.Rand(-0.12,0.12)) + Vector(0,0,-0.1)
