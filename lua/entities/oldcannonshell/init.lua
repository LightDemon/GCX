...
self.ticking = true
self.smoking = false
self.flightvector = self.Entity:GetUp() * 180
...
function ENT:Think()
	...
	if (tr.Hit) then
		if ( self.exploded == false ) then
			...
			if (tr.Entity:IsValid()) then
					...
					local attack = cbt_dealhcghit( tr.Entity, 200, 9, tr.HitPos , tr.HitPos)
						if (attack == 0) then
						brokedshell = ents.Create("prop_physics")
						brokedshell:SetPos(self.Entity:GetPos())
						brokedshell:SetAngles(self.Entity:GetAngles())
						brokedshell:SetKeyValue( "model", "models/props_phx/misc/smallcannonball.mdl" )
    ...
    self.flightvector = self.flightvector + Vector(math.Rand(-3,3), math.Rand(-3,3),math.Rand(-3,3)) + Vector(0,0,-0.4)
