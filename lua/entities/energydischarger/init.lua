...
function ENT:Initialize()   
	...
	util.PrecacheSound( "arty/laser.wav" ) 
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "ISLargeLaser" )
		ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:discharge()
	...
	if (self.ammos > 0) then
		...
		if (tr.Hit && tr.Entity:IsValid()) then
			...
			gcombat.nrghit( tr.Entity, 2000, 5, tr.HitPos, tr.HitPos)
			end
		end
		...
		self.Entity:EmitSound( "arty/laser.wav", 500, 100 )
